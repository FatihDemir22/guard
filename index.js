...
client.on("messageCreate", (message) => {
  if (
    kufurliste.some((k) =>
      message.content.toLowerCase().split(" ").includes(k)
    ) &&
    db.get("kufurEngel")
  )
    message.delete().catch((err) => {});
  else if (
    reklamliste.some((k) => message.content.toLowerCase().includes(k)) &&
    db.get("reklamEngel")
  )
    message.delete().catch((err) => {});
});
...
client.on("channelDelete", async (channel) => {
  ...
  let m = `${String(new Date()).split(" ")[4]} ${
    channel.name
  } isimli kanal silindi.`;
...
app.get("/getInfo", (req, res) => {
  res.status(200).send({
    info: {
      ...
      logs: db.get("logs") || [],
      channels: client.guilds.cache
        .get(process.env.guild)
        .channels.cache.map((r) => r)
        .filter((c) => c.type == "GUILD_TEXT")
        .filter((c) =>
          c.guild.me.permissionsIn(c.id).toArray().includes("MANAGE_CHANNELS")
        ),
...
app.post("/setSlowMode", async (req, res) => {
  ...
  try {
    await client.channels.cache
      .get(channel)
      .setRateLimitPerUser(parseInt(time), "Yavaş Mod");
    db.push(
      "logs",
      `${
        String(new Date()).split(" ")[4]
      } ${channel} ID'li kanala ${time} saniye yavaş mod uygulandı.`
    );
    res.status(200).send({ status: "Başarılı" });
  } catch (err) {
    res
      .status(200)
      .send({ status: "Bir hata oluştu. Konsolda görebilirsiniz", err });
  }
});
...
app.post("/setTimeOut", async (req, res) => {
  ...
  try {
    await client.guilds.cache
      ...
      .timeout(parseInt(time), "Zaman Aşımı");
    res.status(200).send({ status: "Başarılı" });
  } catch (err) {
    res
      .status(200)
